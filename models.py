...
class Actor(nn.Module):
    ...
    def forward(self, x, context_one_hot=None):
        x1 = self.preprocess(x[:, :, :, :320]).view(-1, self.fc_size)
        x2 = self.preprocess(x[:, :, :, 320:640]).view(-1, self.fc_size)
        x3 = self.preprocess(x[:, :, :, 640:]).view(-1, self.fc_size)
...
class Discriminator(nn.Module):
    ...
    def forward(self, state, action, context):
        x1 = self.preprocess(state[:, :, :, :320]).view(-1, self.fc_size)
        x2 = self.preprocess(state[:, :, :, 320:640]).view(-1, self.fc_size)
        x3 = self.preprocess(state[:, :, :, 640:]).view(-1, self.fc_size)
